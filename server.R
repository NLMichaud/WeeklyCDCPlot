...
shinyServer(function(input, output) {
  ...
  output$plot1 <- renderPlot({
    ...
    switch(input$plotty,
          ...
          "cumu"  =  {aesthetics1 = aes(x=date, y=cumulate)
                      aesthetics2 = aes(x=date, y=cumu14)} ,
    ...
    p <- ggplot(selectedData(), aesthetics1)+geom_line(stat="identity",position="identity",size=1)+
         ...
         ggtitle(paste("MMWR",input$disease_name, "Reports for 2014 & 2015"))
    
    if(input$plotty=="week"||input$plotty=="cumu") p <- p + scale_x_date(breaks = "3 month", minor_breaks = "1 month", labels=date_format("%m/%Y"))+xlab("Date")
    ...
    if(input$alert_line){
      ...
      if(input$plotty=="week"||input$plotty=="cumu") p <- p+geom_line( mapping=aesthetics2, linetype="dashed", colour='RED')
...
output$plot2 <- renderPlot({
  ...
  if(input$plottyI=="week"||input$plottyI=="cumu") p <- p + scale_x_date(breaks = "3 month", minor_breaks = "1 month", labels=date_format("%m/%Y"))+xlab("Date")
  
    # If the alert threshold box was checked, include a line on the plots.  Otherwise, plot with no line.
    if(input$alert_lineI){
      p <- p+  geom_point(data=subset(selectedDataI(),alert == T),colour='RED')
      if(input$plottyI=="week"||input$plottyI=="cumu") p <- p+geom_line( mapping=aesthetics2, linetype="dashed", colour='RED')
...
output$plot3 <- renderPlot({
    ...
    if(input$plottyP=="week"||input$plottyP=="cumu") p <- p + scale_x_date(breaks = "3 month", minor_breaks = "1 month", labels=date_format("%m/%Y"))+xlab("Date")
